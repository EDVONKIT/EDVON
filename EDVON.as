...
//			stage.nativeWindow.title += "(" + versionString + ")[测试版12.31]";
			...
			ga = new GATracker(this,"UA-79720504-1","AS3",false);
			track("/app/launch");
			new InvokeMgr();
...
//			ClickerManager.sharedManager().update();
		...
		private function setProjectName(s:String):void {
			if (s.slice(-3) == '.ed') s = s.slice(0, -3);
			if (s.slice(-4) == '.ed2') s = s.slice(0, -4);
...
//				var zipData:ByteArray = projIO.encodeProjectAsZipFile(stagePane);
				...
				if(projectFile != null){
					...
					var defaultName:String = (projectName().length > 1) ? projectName() + '.ed2' : 'project.ed2';
